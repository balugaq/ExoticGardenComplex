...
public abstract class ThreeInputGUI extends SlimefunItem implements EnergyNetComponent {
    ...
    private void constructMenu(BlockMenuPreset preset) {
        for (int i : border) {
            preset.addItem(i, new CustomItemStack(new MaterialData(Material.LEGACY_STAINED_GLASS_PANE, (byte) 3).toItemStack(), " ", new String[0]), (player, i6, itemStack, clickAction) -> false);
        }
        for (int i : inputBorder) {
            preset.addItem(i, new CustomItemStack(new MaterialData(Material.LEGACY_STAINED_GLASS_PANE, (byte) 0).toItemStack(), " ", new String[0]), (player, i5, itemStack, clickAction) -> false);
        }
        ...
        for (int i : subSlotSign) {
            preset.addItem(i, new CustomItemStack(new MaterialData(Material.LEGACY_STAINED_GLASS_PANE, (byte) 5).toItemStack(), "&e副输出槽", new String[]{"", "&7副输出槽通常会输出机器的副产物", "&7有些副产物极其有用甚至非常珍贵"}), (player, i2, itemStack, clickAction) -> false);
        }
        for (int i : mainSlotSign) {
            preset.addItem(i, new CustomItemStack(new MaterialData(Material.LEGACY_STAINED_GLASS_PANE, (byte) 5).toItemStack(), "&c主输出槽", new String[]{"", "&7主输出槽输出机器的常规产品"}), (player, i1, itemStack, clickAction) -> false);
        }
    ...
    public void register(boolean slimefun) {
        addItemHandler((ItemHandler) new BlockTicker() {
